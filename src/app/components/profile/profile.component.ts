...
export class ProfileComponent implements OnInit {
  activeTab: 'personal' | 'security' | 'payments' = 'personal';
  ...
  private setupUserSubscription(): void {
    this.authService.currentUser$.subscribe((user) => {
      if (user) {
        ...
        this.profileImage =
          (user as any).photoUrl || '/assets/icons/perfil_guest.svg';
  ...
  setActiveTab(tab: 'personal' | 'security' | 'payments'): void {
    this.activeTab = tab;
  }
  ...
  removeCard(index: number): void {
    if (
      !confirm('Tem certeza que deseja remover este cartão?') ||
      !this.currentUser
    )
      return;
